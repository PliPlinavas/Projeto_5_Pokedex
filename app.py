...
st.title('Busca pika Ɑ͞ ̶͞ ̶͞ ̶͞ ̶͞ لﮞ͞')

with open('pokemon_index.json', 'r', encoding='utf-8' ) as arquivos:
    nomes_pokemons = json.load(arquivos)
## nome dos pokemons 
nome = st.selectbox("Escolha um Pikachupei:", nomes_pokemons.values())
...
with col1:
    st.metric("A Altura da sua pika é " , f"{altura} M")
with col2:
    st.metric("O peso da sua pika é ", f"{peso} KG")
with col3:
    st.metric("O IMC da sua pika é ", f"{imc}")
